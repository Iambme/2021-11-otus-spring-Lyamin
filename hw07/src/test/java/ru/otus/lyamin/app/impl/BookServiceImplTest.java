...
class BookServiceImplTest {
    ...
    void shouldCorrectlyAddBook() {
        ...
        verify(authorService, times(1)).getAuthorById(getBook().getAuthor().getId());
        verify(genreService, times(1)).getGenreById(getBook().getGenre().getId());
    }
    ...
    void shouldCorrectlyUpdateBook() {
        ...
        verify(bookRepository, times(1)).save(any(Book.class));
        verify(authorService, times(1)).getAuthorById(expectedBook.getAuthor().getId());
        verify(genreService, times(1)).getGenreById(expectedBook.getGenre().getId());
    }

    @DisplayName("корректно удалять книгу ")
    @Test
    void shouldDeleteBookById() {
        bookService.deleteBookById(getBook().getId());
