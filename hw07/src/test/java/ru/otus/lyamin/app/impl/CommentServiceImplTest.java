...
class CommentServiceImplTest {
    ...
    void shouldCorrectlyAddComment() {
        ...
        verify(bookService, times(1)).getBookById(getBook().getId());
    }
    ...
    void shouldCorrectlyUpdateCommentTextById() {
        int expectedCount = 1;
        when(commentRepository.updateCommentTextById(getComment().getId(), getComment().getText())).thenReturn(expectedCount);
        int actualCount = commentService.updateCommentTextById(getComment().getId(), getComment().getText());
        assertThat(actualCount).isEqualTo(expectedCount);
        verify(commentRepository, times(1)).updateCommentTextById(getComment().getId(), getComment().getText());
    }

    @DisplayName("корректно удалять комментарий ")
    @Test
    void shouldDeleteCommentById() {
        commentService.deleteCommentById(getComment().getId());
