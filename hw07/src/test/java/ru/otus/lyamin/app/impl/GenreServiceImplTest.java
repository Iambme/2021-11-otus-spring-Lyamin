...
class GenreServiceImplTest {
    ...
    void shouldCorrectlyUpdateGenre() {
        int expectedCount = 1;
        when(genreRepository.updateGenreNameById(getGenre().getId(), getGenre().getName())).thenReturn(expectedCount);
        int actualCount = genreService.updateGenreNameById(getGenre().getId(), getGenre().getName());
        assertThat(actualCount).isEqualTo(expectedCount);
        verify(genreRepository, times(1)).updateGenreNameById(getGenre().getId(), getGenre().getName());
    }

    @DisplayName("корректно удалять жанр ")
    @Test
    void shouldDeleteGenreById() {
        genreService.deleteGenreById(getGenre().getId());
