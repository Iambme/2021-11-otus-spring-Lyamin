...
class BookControllerTest {
    ...
    @DisplayName("удалять книгу для админа")
    @Test
    @WithMockUser(roles = "ADMIN")
    void shouldCorrectDeleteForAdmin() throws Exception {
        mockMvc.perform(delete("/api/book/1"))
                .andExpect(status().isOk());
    }

    @DisplayName("не удалять книгу для юзера")
    @Test
    @WithMockUser(roles = "USER")
    void shouldNotDeleteForUser() throws Exception {
        mockMvc.perform(delete("/api/book/1"))
